...
def get_usb_iphone():
    ...
    while device is None:
        devices = [
            dev for dev in device_manager.enumerate_devices() if dev.type == Type
        ]
        if len(devices) == 0:
...
def on_message(message, data):
    ...
    if "payload" in message:
        ...
        if "dump" in payload:
            ...
            with SCPClient(
                ssh.get_transport(), progress=progress, socket_timeout=60
            ) as scp:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", dest="output_ipa", help="Specify name of the decrypted IPA"
    )
    ...
    parser.add_argument(
        "-P", "--password", dest="ssh_password", help="Specify SSH password"
    )
    parser.add_argument(
        "-K",
        "--key_filename",
    ...
    parser.add_argument(
        "target", nargs="?", help="Bundle identifier or display name of the target app"
    )
